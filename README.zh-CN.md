...
给编码智能体用的学习教练 skill。把 AI 时代的工作和阅读变成可观测的理解，并且留住它：
复盘刚交付的工作、把一个主题学到可观测的理解程度，再在之后用间隔重复复习，让它真正记住。
全程配套可恢复的检查点。**只能显式调用** —— 绝不自动触发。
...
一次会话里学到「可观测的理解」不是终点 —— teach-me 会把每个已 demonstrated 的概念排期做后续
回忆检查，让它真正记住。想复习就复习，绝不自动开始。

```bash
/teach-me review        # 抽查今天到期的概念
$teach-me review        # Codex / OpenCode
```

- **间隔重复** —— 每个概念沿 Leitner 阶梯前进（`1 → 3 → 7 → 21 → 60` 天）。答上来间隔就拉长；
  没答上就打回并标记 `stale`，teach-me 当场提议重新教一遍。
- **到期而不打扰** —— 只有你开口才会复习。会话结尾或 `resume` 时，若有到期概念，teach-me 只打
  一行安静的提示 —— 不多说，也绝不在非调用场景冒出来。
- **先修优先** —— 概念记录自己依赖什么，复习时先检查基础，再查建立在其上的东西。

复习有批次上限（默认 5 个），并遵守和教学一样的防疲劳规则 —— 一次一个问题，你说累了立刻停。
...
- `tests/` —— 行为场景 + 归档层和存储层的 pytest 测试（用 `uv run --project evals python -m pytest -q tests/` 运行）。
